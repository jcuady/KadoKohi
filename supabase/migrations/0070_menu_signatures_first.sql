-- Espresso Signatures tab before Classics on /menu; keep catalog RPC in sync.
UPDATE public.kk_menu_categories
SET sort_order = CASE id
  WHEN 'cat_signatures' THEN 0
  WHEN 'cat_classics' THEN 1
  WHEN 'cat_matcha' THEN 2
  WHEN 'cat_yuzu' THEN 3
  ELSE sort_order
END,
updated_at = now()
WHERE id IN ('cat_signatures', 'cat_classics', 'cat_matcha', 'cat_yuzu');
...
BEGIN
  ...
  VALUES
    ('cat_signatures', null, 'Espresso Based Signatures', 0, true),
    ('cat_classics', null, 'Espresso Based Classics', 1, true),
