-- Secure order placement: remove guest order enumeration, route all inserts through kk_place_order.

DROP POLICY IF EXISTS kk_orders_select_guest_realtime ON public.kk_orders;
...
CREATE OR REPLACE FUNCTION public.kk_jsonb_option_delta(options jsonb, option_id text)
RETURNS numeric
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT coalesce(
    (
      SELECT (elem->>'priceDelta')::numeric
...
CREATE OR REPLACE FUNCTION public.kk_merch_variant_delta(variants jsonb)
RETURNS numeric
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT coalesce(
    (
      SELECT sum((elem->>'priceDelta')::numeric)
...
CREATE OR REPLACE FUNCTION public.kk_compute_promo_discount(
  p_code public.kk_promo_codes,
  p_subtotal numeric,
  p_lines jsonb
)
RETURNS numeric
...
DECLARE
  ...
  v_drink_count int;
BEGIN
  IF p_code.type = 'percent' THEN
    RETURN least(floor((p_subtotal * p_code.value) / 100), p_subtotal);
  ELSIF p_code.type = 'fixed' THEN
    RETURN least(p_code.value, p_subtotal);
  ELSIF p_code.type = 'free_drink' THEN
    SELECT min((l->>'unit_price')::numeric)
    INTO v_cheapest
    FROM jsonb_array_elements(p_lines) AS l
    WHERE coalesce(l->>'item_type', 'coffee') <> 'merch';
    IF v_cheapest IS NULL THEN
      RETURN 0;
    END IF;
    RETURN least(v_cheapest, p_subtotal);
  ELSIF p_code.type = 'bogo_drink' THEN
    SELECT count(*) INTO v_drink_count
    FROM jsonb_array_elements(p_lines) AS l
    WHERE coalesce(l->>'item_type', 'coffee') <> 'merch';
    IF v_drink_count < 2 THEN
      RETURN 0;
    END IF;
    SELECT min((l->>'unit_price')::numeric)
    INTO v_cheapest
    FROM jsonb_array_elements(p_lines) AS l
    WHERE coalesce(l->>'item_type', 'coffee') <> 'merch';
    RETURN least(coalesce(v_cheapest, 0), p_subtotal);
  END IF;
  RETURN 0;
END;
$$;
...
BEGIN
  ...
  IF v_channel IN ('takeout', 'dine-in') AND v_customer_id IS NULL THEN
  ...
  IF v_loyalty_discount > 0 THEN
    IF v_customer_id IS NULL OR v_customer_id <> v_uid THEN
      RAISE EXCEPTION 'Loyalty discount requires a signed-in customer';
    END IF;
    v_loyalty_discount := least(greatest(v_loyalty_discount, 0), v_subtotal + v_modifiers);
  ...
  v_payment_method := coalesce(
    ...
    CASE WHEN v_channel = 'online' THEN 'gcash-qr' ELSE 'pay-at-store' END
  );
  ...
  INSERT INTO public.kk_orders (
    ...
    v_tax,
    v_total,
    nullif(trim(payload->>'loyalty_voucher_id'), ''),
    nullif(trim(payload->>'loyalty_voucher_code'), ''),
