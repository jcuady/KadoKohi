-- Payment proof storage: owner read/update/delete (required for upsert) + accept proof-storage refs in guest RPC.

-- Storage upsert needs INSERT + SELECT + UPDATE on the same object (Supabase storage RLS).
...
-- Guest folder: allow replace on re-upload.
...
DROP POLICY IF EXISTS "payment_proofs_guest_select" ON storage.objects;
CREATE POLICY "payment_proofs_guest_select"
ON storage.objects FOR SELECT
TO anon, authenticated
USING (
  bucket_id = 'kado-payment-proofs'
