-- Persist coffee cart vs matcha bar on booth proposals.

ALTER TABLE public.kk_booth_bookings
  ADD COLUMN IF NOT EXISTS booking_kind text NOT NULL DEFAULT 'coffee-cart';

ALTER TABLE public.kk_booth_bookings
  DROP CONSTRAINT IF EXISTS kk_booth_bookings_booking_kind_check;

ALTER TABLE public.kk_booth_bookings
  ADD CONSTRAINT kk_booth_bookings_booking_kind_check
  CHECK (booking_kind IN ('coffee-cart', 'matcha-bar'));

UPDATE public.kk_booth_bookings
SET booking_kind = 'matcha-bar'
WHERE booking_kind = 'coffee-cart'
  AND coalesce(special_requests, '') LIKE '%Matcha bar booking%';
