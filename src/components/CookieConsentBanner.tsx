...
/** CSS enter animation — avoids pulling Motion into the deferred consent chunk. */
export default function CookieConsentBanner({ open, onDecide }: Props) {
  ...
  return (
    <div
      ...
      className="cookie-consent-enter fixed z-[200] bottom-[max(1rem,env(safe-area-inset-bottom))] left-[max(1rem,env(safe-area-inset-left))] right-[max(1rem,env(safe-area-inset-right))] sm:right-auto sm:max-w-md pointer-events-auto"
    >
      <div className="rounded-sm border border-kado-cream/40 bg-kado-dark text-kado-cream shadow-2xl shadow-black/40 p-5 sm:p-6">
        <h2 id="cookie-consent-title" className="font-display text-2xl sm:text-[1.65rem] font-bold leading-tight">
          Cookies
        </h2>
        <p id="cookie-consent-desc" className="mt-3 text-sm leading-relaxed text-kado-cream">
          ...
          <Link
            ...
            className="font-semibold underline underline-offset-2 decoration-kado-cream hover:text-white hover:decoration-white"
          >
            Privacy Policy
          </Link>
