  ...
  return (
    ...
    >
          ...
          {ABOUT_TIMELINE.milestones.map((item) => {
            ...
            return (
              <article
                ...
                data-milestone
                className={cn(
                  'w-[min(86vw,19.5rem)] rounded-2xl border p-5 sm:p-6',
                  isHighlight
                    ? 'border-kado-red/40 bg-kado-red text-kado-cream shadow-lg shadow-kado-red/20'
                    : 'border-kado-dark/10 bg-white',
                )}
              >
                <p
                  className={cn(
                ...
                {isHighlight ? (
                  <span className="kado-label mt-4 inline-flex rounded-full border border-kado-cream/30 px-3 py-1 text-kado-cream/90">
                    Now open
                  </span>
                ) : null}
              </article>
      ...
      <ol className="relative hidden lg:block">
        <div
          className="absolute bottom-6 left-1/2 top-4 w-px -translate-x-1/2 bg-kado-dark/10"
          aria-hidden
        />
        <div
          ref={progressRef}
          ...
          return (
            ...
            >
              <div className={cn('pt-1', isLeft ? 'text-right' : 'order-3 text-left')}>
                <article
                  className={cn(
                    'inline-block w-full max-w-md rounded-2xl border p-6 text-left',
                    isHighlight
                      ? 'border-kado-red/30 bg-kado-red text-kado-cream shadow-xl shadow-kado-red/15'
                      : 'border-kado-dark/10 bg-white',
                ...
                >
                  <p
                    className={cn(
                      'kado-label mb-2',
                      isHighlight ? 'text-kado-cream/80' : 'text-kado-red',
                    )}
                  >
                    {item.date}
                  </p>
              ...
              <div className="relative z-10 order-2 flex flex-col items-center pt-1">
                <div
                  className={cn(
                    'flex h-11 w-11 items-center justify-center rounded-full border-2 font-display text-xs font-bold',
                    isHighlight
                      ? 'border-kado-cream bg-kado-red text-kado-cream shadow-md shadow-kado-red/30'
