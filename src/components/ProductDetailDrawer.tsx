...
export default function ProductDetailDrawer({ product, categoryName, onClose }: Props) {
  ...
  // Reset controls whenever the viewed product changes
  useEffect(() => {
    if (!product) return;
  ...
  const selectedMilk = useMemo(
    () => product?.milks?.find((m) => m.id === selectedMilkId),
    [product, selectedMilkId],
  );

  const unitPrice = (product?.basePrice ?? 0) + (selectedMilk?.priceDelta ?? 0);
  ...
  const handleAdd = () => {
    ...
    addItem({
      ...
      temperature: product.temperature === 'both' ? selectedTemp : product.temperature,
      unitPrice,
      lineTotal,
    });
    setAdded(true);
            ...
            <div className="pointer-events-auto w-full sm:max-w-lg bg-white sm:rounded-[2rem] rounded-t-[2.5rem] shadow-[0_30px_60px_rgba(158,24,29,0.15)] overflow-hidden max-h-[93dvh] sm:max-h-[88vh] flex flex-col border sm:border-kado-red/10">
              ...
              <div className="relative shrink-0">
                <div className="aspect-[16/9] overflow-hidden bg-kado-dark/5">
                  <img
                    src={product.image ?? FALLBACK_BY_CATEGORY[product.categoryId] ?? DEFAULT_IMAGE}
                    alt={product.name}
                    ...
                    <div className="flex flex-wrap gap-2">
                      {product.milks.map((m) => (
                        <button
                          key={m.id}
                          type="button"
                          onClick={() => setSelectedMilkId(m.id)}
                        ...
                        >
                          {m.label}
                          {m.priceDelta > 0 && (
                            <span className="ml-1.5 font-bold text-[10px] opacity-80">
                              +₱{m.priceDelta}
                            </span>
                          )}
                        </button>
