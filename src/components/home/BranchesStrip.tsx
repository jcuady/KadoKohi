    ...
    <section className="landing-section relative w-full overflow-hidden border-t border-white/5 bg-kado-dark">
      ...
      >
        <div className="mb-10 flex flex-col gap-6 md:mb-12 md:flex-row md:items-end md:justify-between">
          ...
          <Link
            to="/branches"
            className="kado-label inline-flex min-h-[44px] items-center gap-1 text-kado-cream/60 transition-colors hover:text-white"
          >
            <CmsStyledText
              value={copy.ctaLabel}
              as="span"
          ...
          {branches.map((branch, i) => {
            ...
            return (
              ...
              >
                <Link
                  to="/branches"
                  className="group relative block min-h-[280px] overflow-hidden rounded-2xl border border-white/10 sm:min-h-[320px] sm:rounded-[2rem]"
                >
                  <ResilientImage
                    src={photo}
                  ...
                  <div className="relative flex h-full min-h-[280px] flex-col justify-between p-6 sm:min-h-[320px] sm:p-8">
                    <div>
                      <span
                    ...
                    <div>
                      <h3 className="kado-h3 mb-2 text-kado-offwhite transition-colors group-hover:text-white">
                        {branch.name}
                      </h3>
                      <p className="flex items-start gap-1.5 text-sm text-kado-cream/75">
                      ...
                      {hours && (
                        <p className="mt-2 flex items-center gap-1.5 text-xs text-kado-cream/55">
                          <Clock className="h-3 w-3 shrink-0" aria-hidden />
                          {hours} daily
                        </p>
                      )}
                      <p className="mt-4 inline-flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-kado-red transition-transform group-hover:translate-x-1">
