        ...
        {OPTIONS.map(({ id, label, hint, Icon }) => {
          ...
          return (
            <button
              ...
              type="button"
              onClick={() => onChange(id)}
              className={`min-h-[52px] rounded-xl border px-2 py-2.5 text-left transition-colors touch-manipulation ${
                active
            ...
            >
              ...
              <span className={`mt-0.5 block text-[9px] leading-snug ${active ? 'opacity-90' : 'opacity-60'}`}>
                {hint}
              </span>
            </button>
          );
